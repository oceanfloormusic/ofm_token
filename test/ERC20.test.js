...
const OFMToken = artifacts.require('token/OfmERC20');

contract('ERC20', function (accounts) {
  const [ initialHolder, recipient, anotherAccount ] = accounts;
  const initialSupply = new BN('35000000000000000000000000')
  const name = 'Ocean Floor Music';
  const symbol = '$OFM';

  beforeEach(async function () {
    this.token = await OFMToken.new(initialHolder);
    this.token.updateFeatures(65535);
  });
